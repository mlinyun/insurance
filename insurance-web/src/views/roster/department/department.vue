<template>
    <div class="search">
                ...
                <Form ref="form" :model="form" :label-width="100" :rules="formValidate">
                            ...
                            <div style="display:flex;">
                                ...
                                <Poptip transfer trigger="click" placement="right-start" title="选择上级部门" width="250">
                                    ...
                                    <div slot="content" style="position:relative;min-height:5vh">
                                        <Tree :data="dataEdit" :load-data="loadData" @on-select-change="selectTreeEdit">
                                        </Tree>
                    ...
                    <Row :gutter="16">
                        ...
                        <FormItem label="部门领导" prop="mainHeader">
                            <Select :loading="userLoading" not-found-text="该部门暂无用户数据" v-model="form.mainHeader" multiple
                                filterable placeholder="选择部门领导">
                        ...
                        <FormItem label="部门组长" prop="viceHeader">
                            <Select :loading="userLoading" not-found-text="该部门暂无用户数据" v-model="form.viceHeader" multiple
                                filterable placeholder="选择部门组长">
                                <Option v-for="item in users" :value="item.id" :key="item.id" :label="item.nickname">
                                    <span style="margin-right:10px;">{{ item.nickname }}</span>
                                    <span style="color:#ccc;">{{ item.username }}</span>
                                </Option>
                    ...
                    <Row :gutter="16">
                        ...
                        <Col span="8">
                        <Form-item class="br">
                            <Button @click="submitEdit" :loading="submitLoading" type="success" ghost shape="circle"
                                icon="ios-create-outline" size="small">保存</Button>
                            <Button @click="handleReset" type="warning" ghost shape="circle" size="small"
        ...
        <Card>
            <Row>
                <Table :loading="userLoading" border :columns="userColumns" :data="userData" ref="table" sortable="custom"
                    @on-sort-change="changeSort" @on-selection-change="changeSelect" @on-row-click="rowClick"
                    :row-class-name="rowClassName"></Table>
            </Row>
            <Row type="flex" justify="end" class="page">
                <Page :current="searchForm.pageNumber" :total="userTotal" :page-size="searchForm.pageSize"
                    @on-change="changePage" @on-page-size-change="changePageSize" :page-size-opts="[10, 20, 50]"
                    size="small" show-total show-elevator show-sizer></Page>
            </Row>
        </Card>

        <Modal :title="modalTitle" v-model="addOrEditDepartmentModal" :mask-closable="false" :width="500" on-text="提交"
            @on-ok="submitAdd">

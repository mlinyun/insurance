<template>
    <div class="search">
        <Card>
            <Form ref="searchForm" :model="searchForm" inline :label-width="100">
                <Row class="operation">
                    ...
                    <Button @click="findBackFieldModal = true" type="warning" size="small" ghost shape="circle"
                        icon="ios-construct">读取</Button>
                    <Button @click="generate" :loading="createCodeLoading" size="small" ghost shape="circle" type="success"
                        icon="md-send">生成</Button>
            ...
            <Row>
                ...
                <Table border stripe size="large" :columns="columns" :data="data" ref="table">
                    ...
                    <template slot-scope="{ row }" slot="searchType">
                        <Select v-model="row.searchType" @on-change="changeSearchType(row, $event)"
                            :disabled="!row.searchable" transfer>
                            <Option value="text">文本输入框</Option>
                            <Option value="select">选择下拉框</Option>
                    ...
                    <template slot-scope="{ row }" slot="defaultSort">
                        <i-switch v-model="row.defaultSort" @on-change="changeDefaultSort(row, $event)"
                    ...
                    <template slot-scope="{ row }" slot="defaultSortType">
                        <Select v-model="row.defaultSortType" @on-change="changeDefaultSortType(row, $event)"
                            :disabled="!row.defaultSort || !row.sortable" transfer>
            ...
            <Form ref="entityForm" :model="entityForm" :label-width="130" :rules="entityFormValidate">
                <FormItem label="实体类引用路径" prop="path">
                    <Row type="flex" justify="space-between">
                        <Input v-model="entityForm.path" placeholder="例如：cn.zwz.modules.base.entity.User" clearable
                            style="width:410px" />
                        <Button type="warning" icon="md-play" @click="generateEntityData">读取字段</Button>
                    </Row>
                </FormItem>
            </Form>
            <Input v-model="findBackFieldContent" type="textarea" autosize style="max-height:50vh;overflow:auto;"
                placeholder="生成结果" />
        </Modal>
        ...
        <Modal title="生成代码" v-model="codeModal" :width="1000" styles="top:30px">
            ...
            <div slot="footer">
                <Button v-clipboard:copy="createCodeContent" v-clipboard:success="onCopy" type="primary"
                    icon="md-copy">复制代码</Button>
            </div>
        </Modal>
    </div>
</template>

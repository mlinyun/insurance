package com.mlinyun.insurance.basics.redis;
...
import com.mlinyun.insurance.data.vo.HttpIpSsoSetting;
...
public class LimitRaterInterceptor extends HandlerInterceptorAdapter {
    ...
    public HttpIpSsoSetting getHttpIpSsoSetting() {
        ...
        if (s != null && !InsNullUtils.isNull(s.getValue())) {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, @ApiParam(name = "响应的处理器") Object handler) throws Exception {
        ...
        // 单IP限流判断
        if (insLoginProperties.getOneLimiting()) {
            boolean flag1 = redisRaterLimiter.getLimitFlag(ip, insLoginProperties.getOneLimitingSize(), insLoginProperties.getOneLimitingTime());
            if (!flag1) {
                throw new InsException("您的请求过于频繁，请稍后再试！");
            }
        }
        // 全局限流判断
        if (insLoginProperties.getAllLimiting()) {
            boolean flag2 = redisRaterLimiter.getLimitFlag("ZWZ_LIMIT_ALL", insLoginProperties.getAllLimitingSize(), insLoginProperties.getAllLimitingTime());
            if (!flag2) {
                throw new InsException("系统已达到最大承载量，无法继续提供服务，请稍后再试！");
            }
        }
        // IP黑名单
        HttpIpSsoSetting os = getHttpIpSsoSetting();
        if (os != null && !InsNullUtils.isNull(os.getBlacklist())) {
            String[] list = os.getBlacklist().split("\n");
            for (String item : list) {
                if (Objects.equals(ip, item)) {
                    throw new InsException("您已被禁止访问该系统，如有疑问请咨询管理员，谢谢！");
                }
            }
        }
        // 特定方法限流判断
        try {
            ...
            if (rateLimiter != null) {
                ...
                if (!flag3) {
                    throw new InsException(method.getName() + "方法请求超限，请稍后再试");
                }
            }
        } catch (Exception e) {
        }

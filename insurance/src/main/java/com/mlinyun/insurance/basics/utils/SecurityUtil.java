package com.mlinyun.insurance.basics.utils;
...
public class SecurityUtil {
    ...
    @Autowired
    private InsLoginProperties tokenProperties;
    ...
    public String getToken(String username, Boolean saveLogin) {
        if (InsNullUtils.isNull(username)) {
            throw new InsException("username不能为空");
        }
        boolean saved = false;
        ...
        if (tokenProperties.getSaveRoleFlag()) {
            for (PermissionDTO p : selectUser.getPermissions()) {
                if (!InsNullUtils.isNull(p.getTitle()) && !InsNullUtils.isNull(p.getPath())) {
        ...
        if (tokenProperties.getSsoFlag()) {
            String oldToken = redisTemplate.opsForValue().get(InsLoginProperties.USER_TOKEN_PRE + selectUser.getUsername());
            if (StrUtil.isNotBlank(oldToken)) {
                redisTemplate.delete(InsLoginProperties.HTTP_TOKEN_PRE + oldToken);
            }
        }
        // 保存至Redis备查
        if (saved) {
            redisTemplate.opsForValue().set(InsLoginProperties.USER_TOKEN_PRE + selectUser.getUsername(), ansUserToken, tokenProperties.getUserSaveLoginTokenDays(), TimeUnit.DAYS);
            redisTemplate.opsForValue().set(InsLoginProperties.HTTP_TOKEN_PRE + ansUserToken, new Gson().toJson(tokenUser), tokenProperties.getUserSaveLoginTokenDays(), TimeUnit.DAYS);
        } else {
            redisTemplate.opsForValue().set(InsLoginProperties.USER_TOKEN_PRE + selectUser.getUsername(), ansUserToken, tokenProperties.getUserTokenInvalidDays(), TimeUnit.MINUTES);
            redisTemplate.opsForValue().set(InsLoginProperties.HTTP_TOKEN_PRE + ansUserToken, new Gson().toJson(tokenUser), tokenProperties.getUserTokenInvalidDays(), TimeUnit.MINUTES);
        }
        return ansUserToken;
    }
